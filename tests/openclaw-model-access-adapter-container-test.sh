...
fail() { printf 'FAIL: %s\n' "$1" >&2; exit 1; }
mkdir -p "$WORK_DIR/runtime" "$WORK_DIR/data"
...
run_adapter() {
  docker run --rm --platform linux/amd64 \
    ...
    -v "$WORK_DIR/runtime:/run/labnow/model-access:ro" \
...
jq -e '.phase == "ready" and has("api_key") | not' "$WORK_DIR/data/labnow-model-access/status.json" >/dev/null || fail "status is invalid or contains a key"
