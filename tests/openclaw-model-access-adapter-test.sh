...
CONTRACT_DIR="${MODEL_ACCESS_FIXTURES_DIR:-$REPO_ROOT/../lab_project_analysis/contracts/model-access/v1alpha1/fixtures}"
[[ -d "$CONTRACT_DIR" ]] || { printf 'FAIL: contract fixtures not found at %s; set MODEL_ACCESS_FIXTURES_DIR\n' "$CONTRACT_DIR" >&2; exit 1; }
WORK_DIR="$(mktemp -d)"
